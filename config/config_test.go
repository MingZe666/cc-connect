...
func TestWorkspaceAutoCreateOptIn(t *testing.T) {
	for _, enabled := range []bool{false, true} {
		text := "[[projects]]\nname = 'test'\nmode = 'multi-workspace'\n"
		if enabled {
			text += "workspace_auto_create = true\n"
		}
		var c Config
		if _, err := toml.Decode(text, &c); err != nil {
			t.Fatal(err)
		}
		if len(c.Projects) != 1 || c.Projects[0].WorkspaceAutoCreate != enabled {
			t.Fatalf("auto-create config mismatch: enabled=%v", enabled)
		}
	}
}
