...
type codexSession struct {
	...
	cliExtraArgs   []string // extra args from cmd; config overrides belong to the exec/resume scope
	extraEnv       []string
...
func (cs *codexSession) buildExecArgs(prompt string, imagePaths []string) []string {
	...
	// Config overrides must share the exec/resume scope with generated -c flags.
	// Codex otherwise drops root-level overrides when subcommand overrides exist.
	var rootArgs []string
	for i := 0; i < len(cs.cliExtraArgs); i++ {
		arg := cs.cliExtraArgs[i]
		if arg == "-c" || arg == "--config" {
			args = append(args, arg)
			if i+1 < len(cs.cliExtraArgs) {
				i++
				args = append(args, cs.cliExtraArgs[i])
			}
