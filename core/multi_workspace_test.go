...
func TestMultiWorkspaceAutoCreate_EmptyNameAndStableIsolation(t *testing.T) {
	base := t.TempDir()
	e := newTestEngineWithMultiWorkspace(t, base)
	e.SetWorkspaceAutoCreate(true)
	p := &mockChannelResolver{names: map[string]string{"a": "", "b": ""}}
	first, _, err := e.resolveWorkspace(p, "a")
	if err != nil || first == "" {
		t.Fatalf("first message must get a workspace without confirmation: path=%q err=%v", first, err)
	}
	if first == normalizeWorkspacePath(base) {
		t.Fatal("must not bind unnamed chat to base directory")
	}
	if info, err := os.Stat(first); err != nil || !info.IsDir() {
		t.Fatalf("directory not created: %v", err)
	}
	again, _, err := e.resolveWorkspace(p, "a")
	if err != nil || again != first {
		t.Fatal("same chat must reuse its directory")
	}
	other, _, err := e.resolveWorkspace(p, "b")
	if err != nil || other == "" || other == first {
		t.Fatal("different chats must have distinct directories")
	}
	reloaded := NewWorkspaceBindingManager(e.workspaceBindings.storePath)
	if reloaded.Lookup("project:test", workspaceChannelKey(p.Name(), "a")) == nil {
		t.Fatal("binding not persisted")
	}
}

func TestMultiWorkspaceAutoCreate_DuplicateAndUnsafeNames(t *testing.T) {
	e := newTestEngineWithMultiWorkspace(t, t.TempDir())
	e.SetWorkspaceAutoCreate(true)
	p := &mockChannelResolver{names: map[string]string{"a": "../../outside", "b": "../../outside"}}
	a, _, err := e.resolveWorkspace(p, "a")
	if err != nil || a == "" {
		t.Fatalf("resolve a: %q %v", a, err)
	}
	b, _, err := e.resolveWorkspace(p, "b")
	if err != nil || a == b {
		t.Fatalf("same names must not share a workspace: %q %q %v", a, b, err)
	}
	if filepath.Dir(filepath.FromSlash(a)) != filepath.Clean(e.baseDir) {
		t.Fatalf("workspace escaped base: %q", a)
	}
}

func TestMultiWorkspaceAutoCreate_FailureDoesNotBind(t *testing.T) {
	base := filepath.Join(t.TempDir(), "file")
	if err := os.WriteFile(base, []byte("occupied"), 0600); err != nil {
		t.Fatal(err)
	}
	e := newTestEngineWithMultiWorkspace(t, base)
	e.SetWorkspaceAutoCreate(true)
	p := &mockChannelResolver{}
	ws, _, err := e.resolveWorkspace(p, "a")
	if err == nil || ws != "" {
		t.Fatalf("creation failure must surface: %q %v", ws, err)
	}
	if e.workspaceBindings.Lookup("project:test", workspaceChannelKey(p.Name(), "a")) != nil {
		t.Fatal("failed creation was bound")
	}
}

func TestMultiWorkspaceAutoCreate_PreservesBindingAndRejectsEmptyID(t *testing.T) {
	e := newTestEngineWithMultiWorkspace(t, t.TempDir())
	e.SetWorkspaceAutoCreate(true)
	p := &mockChannelResolver{}
	existing := normalizeWorkspacePath(t.TempDir())
	e.workspaceBindings.Bind("project:test", workspaceChannelKey(p.Name(), "a"), "existing", existing)
	got, _, err := e.resolveWorkspace(p, "a")
	if err != nil || got != existing {
		t.Fatalf("existing binding changed: %q %v", got, err)
	}
	if got, _, err := e.resolveWorkspace(p, ""); err == nil || got != "" {
		t.Fatalf("empty ID accepted: %q %v", got, err)
	}
}

func TestMultiWorkspaceAutoCreate_RecoversMissingDirectory(t *testing.T) {
	e := newTestEngineWithMultiWorkspace(t, t.TempDir())
	e.SetWorkspaceAutoCreate(true)
	p := &mockChannelResolver{}
	e.workspaceBindings.Bind("project:test", workspaceChannelKey(p.Name(), "a"), "deleted", filepath.Join(e.baseDir, "deleted"))
	ws, _, err := e.resolveWorkspace(p, "a")
	if err != nil || ws == "" {
		t.Fatalf("missing binding must recover without prompting: %q %v", ws, err)
	}
}
