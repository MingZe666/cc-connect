...
func TestCUJ_H5_AutoWorkspaceProcessesFirstMessage(t *testing.T) {
	env := newCUJEnv(t)
	env.engine.SetMultiWorkspace(filepath.Join(env.tempDir, "workspaces"), filepath.Join(env.tempDir, "bindings.json"))
	env.engine.SetWorkspaceAutoCreate(true)
	env.engine.SetSkipGit(true)
	previous, existed := agentFactories["cuj"]
	t.Cleanup(func() {
		if existed {
			agentFactories["cuj"] = previous
		} else {
			delete(agentFactories, "cuj")
		}
	})
	RegisterAgent("cuj", func(map[string]any) (Agent, error) { return env.agent, nil })
	for i, channel := range []string{"chat-a", "chat-b", "chat-a"} {
		before := len(env.plat.getSent())
		env.engine.ReceiveMessage(env.plat, &Message{SessionKey: "test:" + channel + ":user", Platform: "test", ChannelKey: channel, UserID: "user", UserName: "user", MessageID: fmt.Sprintf("auto-%d", i), Content: "process report", ReplyCtx: channel})
		env.waitFor("report processed without initialization question", 3*time.Second, func() bool {
			sent := env.plat.getSent()
			for _, reply := range sent[before:] {
	...
	for _, reply := range env.plat.getSent() {
		if strings.Contains(reply, "mkdir") || strings.Contains(reply, "yes/no") {
			t.Fatalf("unexpected initialization question: %q", reply)
		}
	}
}
