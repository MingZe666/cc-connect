...
// SetWorkspaceAutoCreate enables deterministic per-chat directories without confirmation.
func (e *Engine) SetWorkspaceAutoCreate(enabled bool) {
	e.workspaceAutoCreate = enabled
}
...
func (e *Engine) resolveWorkspace(p Platform, channelID string) (string, string, error) {
	...
	if b, _, usable := e.lookupEffectiveWorkspaceBinding(channelKey); b != nil {
		...
		if !e.workspaceAutoCreate {
			return "", b.ChannelName, nil
		}
	}

	// Explicit opt-in uses opaque IDs, so empty/renamed/duplicate channel names
	// cannot merge workspaces or inject filesystem path components.
	if e.workspaceAutoCreate {
		if strings.TrimSpace(channelID) == "" {
			return "", "", fmt.Errorf("auto-create workspace: empty channel ID")
		}
		digest := sha256.Sum256([]byte(e.name + "\x00" + channelKey))
		folder := "chat-" + hex.EncodeToString(digest[:16])
		candidate := filepath.Join(e.baseDir, folder)
		if err := os.MkdirAll(candidate, 0o700); err != nil {
			return "", "", fmt.Errorf("auto-create workspace: %w", err)
		}
		base, err := filepath.EvalSymlinks(e.baseDir)
		if err != nil {
			return "", "", fmt.Errorf("resolve workspace base: %w", err)
		}
		resolved, err := filepath.EvalSymlinks(candidate)
		if err != nil {
			return "", "", fmt.Errorf("resolve auto-created workspace: %w", err)
		}
		if normalizeWorkspacePath(filepath.Dir(resolved)) != normalizeWorkspacePath(base) {
			return "", "", fmt.Errorf("auto-created workspace escapes base directory")
		}
		normalized := normalizeWorkspacePath(resolved)
		e.workspaceBindings.Bind("project:"+e.name, channelKey, folder, normalized)
		return normalized, folder, nil
	}
