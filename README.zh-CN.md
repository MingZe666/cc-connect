...
默认情况下，忙碌会话的新消息进入下一轮队列。若使用 Codex 本地 App Server，可在目标项目已有的 `[projects.agent.options]` 中加入：

```toml
# 使用本地标准输入输出连接。
backend = "app_server"
app_server_url = "stdio://"
# 默认值为 queue，steer 表示优先补充当前轮次。
busy_message_mode = "steer"
```

保留现有 `cmd`、`mode`、模型、网络访问参数和多工作区配置。补充成功会回复“已补充到当前任务”，不会额外创建一轮；尚在启动或没有活跃轮次时回退排队。其他 Agent 未实现此能力时仍使用原队列。

确认超时或连接中断后，消息可能已经被接收，因此不会自动重发。可通过 `/history` 查看待核实记录，并结合当前任务结果判断；重启也不会自动重放这类消息。补充只影响后续执行，不撤销此前完成的操作。配置变更需要重启，生产切换应在当前任务结束和独立实例验证通过后进行。
